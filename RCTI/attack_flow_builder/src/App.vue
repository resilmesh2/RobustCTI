<template>
  <AppHotkeyBox id="main">
    ...
    <IntegrationToolsDialog id="integration-tools-dialog" />
...
import IntegrationToolsDialog from "@/components/Elements/IntegrationToolsDialog.vue";
...
export default defineComponent({
  ...
  computed: {
     ...
     */
    ...mapState("ApplicationStore", ["settings", "activePage"]),
  ...
  async created() {
    ...
    // Load settings
    this.execute(new App.LoadSettings((this as any).$store.state.ApplicationStore, settings));
    // Load empty file
    this.execute(await App.LoadFile.fromNew((this as any).$store.state.ApplicationStore));
    ...
    if(src) {
      try {
        ...
        this.execute(await App.PrepareEditorWithFile.fromUrl((this as any).$store.state.ApplicationStore, src));
      } catch(ex) {
