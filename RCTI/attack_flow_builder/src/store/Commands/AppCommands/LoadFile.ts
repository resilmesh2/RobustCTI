...
export class LoadFile extends AppCommand {
    ...
    /**
     * Loads an empty IoB page file into the application.
     * @param context
     *  The application context.
     * @returns
     *  A Promise that resolves with the {@link LoadFile} command.
     */
    public static async fromNewIoB(context: ApplicationStore): Promise<LoadFile> {
        let schema = structuredClone(Configuration.schema);
        // Set the flow type to IoB before creating the page
        if (schema.templates[0]?.properties?.flow_type) {
            (schema.templates[0].properties.flow_type as any).value = "IoB";
        }
        let page = await PageEditor.createNew(schema);
        return new LoadFile(context, page);
    }
