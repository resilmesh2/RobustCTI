...
export class PrepareEditorWithFile extends GroupCommand {
    ...
    /**
     * Prepares the editor with an empty IoB page.
     * @param context
     *  The application context.
     * @returns
     *  A Promise that resolves with the {@link PrepareEditorWithFile} command.
     */
    public static async fromNewIoB(context: ApplicationStore): Promise<PrepareEditorWithFile> {
        return new PrepareEditorWithFile(context, await LoadFile.fromNewIoB(context));
    }
